...
logmeandist = np.log(meandist)

sparsity = (meandist.size-np.count_nonzero(meandist[meandist>0.000015]))/meandist.size
sparsity_log= (logmeandist.size-np.count_nonzero(logmeandist[logmeandist>0.000015]))/logmeandist.size

print('Sparsity(non-zero/size) {} {} '.format(sparsity, sparsity_log))
y_pred =np.argmax(np.around(Sts), axis=1).astype(int).reshape(T_test.shape[0],1)
